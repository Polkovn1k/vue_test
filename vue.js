...
const App = {
  template: `
    <div class="card">
      ...
      <div class="steps">
        ...
        <ul class="steps-list" :style="setListPreventStyle">
          <li v-for="(item, i) in steps"
              :key="item.title"
              class="steps-item" 
        ...
        <div>
          <template v-if="!restart">
            <button class="btn" :disabled="isPrevBtnDisabled" @click="prev">Назад</button>
            <button class="btn primary" @click="nextOfFinish">{{ setNextBtnInnerText }}</button>
          </template>
          <template v-else>
  ...
  computed: {
    ...
    setNextBtnInnerText() {
      return this.isLastStepActive ? 'Закончить' : 'Вперед';
    },
    setListPreventStyle() {
      return {
        pointerEvents: this.restart ? 'none' : 'auto',
      }
    },
  }
}
